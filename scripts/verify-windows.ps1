...
function Install-Panel {
  ...
  if (!(Test-Path "$installDir/Dynamic Panel.exe")) { throw 'Installed executable missing' }
}
Install-Panel
node scripts/smoke-app.js "$installDir/Dynamic Panel.exe" $profile
if ($LASTEXITCODE -ne 0) { throw 'Installed application smoke failed' }
Install-Panel
node scripts/smoke-app.js "$installDir/Dynamic Panel.exe" $profile retained
...
if (Test-Path "$installDir/Dynamic Panel.exe") { throw 'Uninstall left executable behind' }
