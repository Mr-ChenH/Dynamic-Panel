...
首页 AI 临时对话复用「设置 → AI 与转写」中的默认内容模型，并在顶栏显示当前厂商和模型；只在主动发送时调用，支持安全 Markdown、代码复制、停止、原位重试、新建对话、复制回复和手动保存为笔记。停止或失败会保留当前轮次以便检查和重试，但不完整结果不会进入后续上下文；成功回答最多保留最近 6 轮，总输入仍限制 12,000 字符。对话仅存在于当前窗口内存，不自动读取工作区、联网搜索或执行操作，也不会自动保存到笔记。
