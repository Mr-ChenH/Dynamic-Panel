...
| **行情（开发中）** | 与首页平级的市场观察工作区；加密市场由 CoinGecko / Binance 提供，美股支持 Alpha Vantage 日终榜单、Alpaca IEX / SIP 快照、可选 Twelve Data 报价/搜索/历史及 SEC EDGAR 申报基本面；未配置的数据源显示明确空态 |
| **笔记** | 独立新建、自动保存、全文搜索，以及“分类 → 标签”两级整理；支持 Markdown 编辑与预览，以及粘贴、拖入或选择本地图片，附件随工作区保存 |
| **链接** | 保存公开网址，后台补全标题、描述、图标和分组；支持标签、收藏、已读状态、稍后阅读筛选与高级搜索 |
| **录制** | 录音开始即创建实时记录，同步显示状态与转写，并可在页内配置 API |
| **密钥** | 使用系统安全存储加密账号、密码和 API Key |
| **设置** | 当菜单栏图标被刘海遮挡时，仍可在面板内配置 API、金融自选与展示偏好、功能显示、默认展开页、快捷键、数据目录与开机启动 |
...
金融观察页采用只读浏览边界和按需页面结构：总览首屏固定并列展示 A 股、美股与加密货币的连接状态和样本摘要，主图只绘制 provider 实际返回的历史序列，下方按三个市场分别组合领涨、领跌和成交活跃标的；未配置或不支持榜单的市场保留明确空态，不会用加密货币数据替代。榜单占用完整内容区，每页最多展示 50 行并提供上一页/下一页，自选页才显示分组、报价和详情；加密榜单在同一市场下切换 CoinGecko 聚合或 Binance USDT 现货来源，点击榜单项目可直接切换到该项目详情，不会自动加入自选；不再保留常驻侧栏，也不会生成模拟价格、指数、市场宽度、板块或走势图。总览支持用户主动点击的 AI 快照解读：模型只接收本次行情快照，按“市场语气 / 简短结论 / 正负与分化信号 / 后续观察”结构返回，每条信号都带可在快照中核对的证据；行情更新后旧解读会保留在当前窗口中并明确标记为已失效，用户重新生成成功后才替换；解读不会持久化，结果明确不构成投资建议。资产搜索、加入/移除、分组管理、默认视图、默认榜单与刷新频率统一位于「设置 → 金融与行情」。自选关系存入 `notch-finance-watchlists-v1`，展示偏好存入 `notch-finance-view-preferences-v1`；升级时会清除旧版未确认的内置 fixture，并从保留的资产身份中移除旧行情字段。
...
启动器提供「摘要文字 / 精简文字 / 翻译文字 / 从文字提取待办」，待办页可直接从文字生成候选，笔记和录音详情也提供就地整理入口。链接保存后会补全网页描述；启用链接自动整理时，AI 还会基于网址、标题和描述建议名称、分类与标签，需由用户确认；笔记、录音和链接还可手动生成名称或分类；内容先进入统一预览，用户检查、编辑后再复制、采用名称、另存笔记或加入待办。文本动作支持服务端 SSE 增量显示；普通搜索、打开页面和录音保存不会自动触发这些新整理请求。
...
- **数据留在本机**：待办、笔记、链接、金融自选、录音元数据和工作区设置保存在本地，无后端和云同步；笔记图片保存到工作区的 `note-images/`，正文只记录可迁移的相对路径。
...
[金融行情扩展研究](docs/finance-extension-research.md)、[实时数据源和全市场展示研究](docs/finance-market-data-and-display-research.md) 与 [金融页 AI 解读研究](docs/finance-ai-interpretation-research.md) 记录了跨市场资产身份、只读行情/设置管理边界、数据授权、刷新策略、证据约束和 `FinanceService` 接入方案。
