...
打开「录制」页，点击「新建截图」后，应用会立即隐藏面板并覆盖鼠标所在的整个屏幕；按住鼠标拖动框选，松开即完成截图并自动复制到系统剪贴板，同时保存为 PNG 到资料库。截图过程中按 Esc 可取消。录屏仍支持选择整个屏幕、单个窗口或区域。

Windows 的窗口来源使用名称列表，不为所有窗口预先采集缩略图；最小化、隐藏和检测到禁止采集的窗口不会列出。窗口在选择后关闭或采集被系统拒绝时，会提示恢复窗口、刷新重选，也可手动改选整个屏幕，应用不会自动切换采集目标。

录屏的「框选区域」方式会在选择一个屏幕后显示冻结画面，可拖动框选或输入相对画面左上角的 X / Y / 宽 / 高（像素，至少 2 × 2）。勾选「记住此区域」后，下次在相同显示器、分辨率和缩放下自动预填，仍可重新拖动调整，确认后才开始；录屏可在「设置 → 截图与录屏」清除记住的区域。显示器或实际画面尺寸改变时要求重新选择，不自动扩大到整屏，也不支持跨屏拼接。区域录屏只编码所选矩形，画质上限作用于裁剪后的输出；录制中尺寸变化会停止并保留已录内容。

录屏支持整个屏幕、单个窗口或框选区域，默认无声；可在「设置 → 截图与录屏」配置默认采集方式、主动选择麦克风，并设置画质和 0 / 3 / 5 秒倒计时。录屏保存为 WebM，切换页面或折叠主面板不会终止；从托盘菜单或面板顶部停止。采集选择器和主面板会在开始前隐藏，录制时主动展开主面板可能入镜。暂不提供系统声音、标注、全局截图快捷键或 MP4 转码。
