...
面板默认每次从首页展开。可在「设置 → 通用」中更改默认展开页，并统一配置面板展开/收起、搜索启动器、区域截图和录音开始/停止四类全局快捷键。面板默认使用悬停时生效的 `Space`，启动器默认使用 `Cmd/Ctrl+Space`；截图与录音默认不占用快捷键，可设置包含修饰键的组合或随时禁用。若组合键与应用内其他操作或系统快捷键冲突，原设置会保留。若后续隐藏了被选中的默认页，下次展开会自动回退到首页。
...
录屏支持整个屏幕、单个窗口或框选区域，默认无声；可在「设置 → 截图与录屏」配置默认采集方式、主动选择麦克风，并设置画质和 0 / 3 / 5 秒倒计时。录屏保存为 WebM，切换页面或折叠主面板不会终止；浮动控制条、托盘菜单和面板顶部均可停止保存或取消丢弃。采集选择器和主面板会在开始前隐藏，录制时主动展开主面板可能入镜。区域截图可在「设置 → 通用」配置全局快捷键；暂不提供系统声音或 MP4 转码。
