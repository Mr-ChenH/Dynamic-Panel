...
| **笔记** | 独立新建、自动保存、全文搜索、Markdown 编辑与预览；支持粘贴、拖入或选择本地图片，附件随工作区保存 |
...
「设置 → AI 与转写」继续使用 OpenAI-compatible 的 Base URL、模型和系统加密 API Key，并增加请求超时、文本与结构化连接测试、最近 50 条脱敏请求诊断，以及笔记、录音、链接三个独立自动命名开关。新整理单次最多处理 12000 字符；Key 不会进入工作区或渲染页面，但所选文字会发送到用户配置的服务商。录音音频仍先保存在本机，整理请求只发送已保存的转写文字。
