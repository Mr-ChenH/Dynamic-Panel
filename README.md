...
| **录制** | 录音、截图与录屏工作区；截图支持类似系统工具的全屏覆盖框选并自动复制到剪贴板，录屏支持整屏、窗口和区域采集 |
...
面板默认每次从首页展开。可在「设置 → 通用」中更改默认展开页，并统一配置面板展开/收起、搜索启动器、区域截图、区域录屏和录音开始/停止五类全局快捷键。面板默认使用悬停时生效的 `Space`，启动器默认使用 `Cmd/Ctrl+Space`；截图、录屏与录音默认不占用快捷键，可设置包含修饰键的组合或随时禁用。若组合键与应用内其他操作或系统快捷键冲突，原设置会保留。若后续隐藏了被选中的默认页，下次展开会自动回退到首页。

## 截图与录屏（开发中，尚未发布）

打开「录制」页，点击「新建截图」后，应用会立即隐藏面板并覆盖鼠标所在的整个屏幕；按住鼠标拖动框选，松开后进入截图编辑，可添加矩形、箭头、画笔、文字和马赛克，支持颜色、粗细及撤销 / 重做。点击完成后自动复制到系统剪贴板，同时保存为 PNG 到资料库；按 Esc 可取消。资料库中的已有截图可通过「编辑标注」再次打开，完成后原位更新并保留名称和条目 ID。录屏仍支持选择整个屏幕、单个窗口或区域。

Windows 的窗口来源使用名称列表，不为所有窗口预先采集缩略图；最小化、隐藏和检测到禁止采集的窗口不会列出。窗口在选择后关闭或采集被系统拒绝时，会提示恢复窗口、刷新重选，也可手动改选整个屏幕，应用不会自动切换采集目标。

录屏的「框选区域」方式会在选择一个屏幕后显示冻结画面，可拖动框选或输入相对画面左上角的 X / Y / 宽 / 高（像素，至少 2 × 2）。勾选「记住此区域」后，下次在相同显示器、分辨率和缩放下自动预填，仍可重新拖动调整，确认后才开始；录屏可在「设置 → 截图与录屏」清除记住的区域。确认区域后立即以红色边框标示实际范围，并显示准备、倒计时与录制状态控制条，可选择停止并保存或取消并丢弃。边框优先位于裁剪范围外且鼠标穿透，提示窗口同时启用系统内容保护。显示器或实际画面尺寸改变时要求重新选择，不自动扩大到整屏，也不支持跨屏拼接。区域录屏只编码所选矩形，画质上限作用于裁剪后的输出；录制中尺寸变化会停止并保留已录内容。

录屏支持整个屏幕、单个窗口或框选区域，默认无声；可在「设置 → 截图与录屏」配置默认采集方式、主动选择麦克风，并设置画质和 0 / 3 / 5 秒倒计时。录屏保存为 WebM，切换页面或折叠主面板不会终止；浮动控制条、托盘菜单和面板顶部均可停止保存或取消丢弃。采集选择器和主面板会在开始前隐藏，录制时主动展开主面板可能入镜。区域截图和区域录屏均可在「设置 → 通用」配置全局快捷键；录屏快捷键只对本次任务强制使用区域模式，确认屏幕来源后直接进入框选，不修改保存的默认采集方式。暂不提供系统声音或 MP4 转码。

截图和录屏保存在当前数据文件夹的 `captures/screenshots/`、`captures/videos/`，元数据由主进程写入 `captures/index.json`，不把二进制存入 LocalStorage。录屏按块落盘，单次上限为 60 分钟或 4 GiB；异常中断时保留为「未完成」，可导出尝试恢复，但不保证可播放。录音与画面采集互斥，采集中不能切换数据文件夹；锁屏、休眠或来源关闭时停止采集，解锁后不会自动恢复。macOS 需要用户授予屏幕录制权限，麦克风仅在主动开启时请求。

`npm run test:capture` 使用生成画面验证采集、后台编码和回放，不读取真实桌面或麦克风；真实 macOS 权限、不同系统版本及混合 DPI 多屏仍需设备验收。
...
启动器支持中文原文、拼音全拼、拼音首字母和别名搜索，并继续支持英文子序列匹配；提供「摘要文字 / 精简文字 / 翻译文字 / 从文字提取待办」，待办页可直接从文字生成候选，笔记和录音详情也提供就地整理入口。链接保存后会补全网页描述；启用链接自动整理时，AI 还会基于网址、标题和描述建议名称、分类与标签，需由用户确认；笔记、录音和链接还可手动生成名称或分类；内容先进入统一预览，用户检查、编辑后再复制、采用名称、另存笔记或加入待办。文本动作支持服务端 SSE 增量显示；普通搜索、打开页面和录音保存不会自动触发这些新整理请求。
