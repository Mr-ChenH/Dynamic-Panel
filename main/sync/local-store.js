...
function createSyncLocalStore({ fsModule, pathModule, filePath, workspaceId = 'default', randomUUID = crypto.randomUUID } = {}) {
  ...
  return Object.freeze({
    ...
    completeInbox(batchId, cursor) { return update((_draft, bucket) => { bucket.inbox = bucket.inbox.filter((row) => row.batchId !== batchId); const next = cursor ? clone(cursor) : bucket.cursor; if (next) bucket.cursor = { ...next, savedAt: Date.now() }; }); },
    cursor: () => clone(active().cursor),
    saveCursor(cursor) { return update((_draft, bucket) => { bucket.cursor = { ...(clone(cursor) || {}), savedAt: Date.now() }; }); },
