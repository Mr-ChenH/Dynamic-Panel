...
- 可见 Tab：首页 bento（可选百炼实时转写的快速录音 / 图标式当前窗口 / Markdown 速记 / 常用指令 / 汽水音乐 / 番茄钟）+ 待办 2 × 2 矩阵（内部键 P0–P3，默认显示名为学习与课程 / 内容与创作 / 产品与开发 / 生活与事务且可改；今天 / 本周 / 以后 / 全部由 deadline 自动派生，逾期归入今天，完成项折叠）+ 笔记管理 + 链接分组 + 录制管理 + 密钥。剪贴板默认关闭、可从菜单栏「显示功能」启用
