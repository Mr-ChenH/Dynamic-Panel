...
## 安全与生命周期
天气搜索参数编码，不接受任意 URL；超时与响应限制；错误脱敏；不自动定位。音乐文件由系统对话框授权，支持扩展名、大小和普通文件校验；网络音乐仅允许无凭据的公开 HTTPS URL，拒绝本机/私网解析和重定向，请求绑定验证后的公网地址并按流限制 128 MB。渲染层切换或删除当前来源、页面卸载时停止音频并撤销 Blob URL。AI沿用安全传输、全应用并发与取消；工作区/配置变更使当前对话请求过期，不自动读取本地资料。清空会话取消请求，迟到回复不回填。

## 验证
领域单测覆盖天气输入/缓存/错误、音乐库持久化和网络边界、聊天历史与角色边界；Electron场景覆盖导航、收集保存、天气假服务、应用内 WAV 加载/播放、AI fake provider、取消和布局。真实本地格式和远程服务器兼容性需实机验收，不能用假服务代替。
