...
- `renderer/chat-context.js` / `renderer/chat-sessions.js` / `ai/schema.js` / `prompts.js` / `service.js`：共享资料规范、消息封装和工作区会话 schema，chat 使用有界 role/content 历史；OpenAI 与 Anthropic 适配均传递真实角色消息。
