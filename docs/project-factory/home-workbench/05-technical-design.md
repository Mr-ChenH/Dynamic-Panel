...
- home-services.js：天气固定 HTTPS API 请求与缓存。home-media.js：以兼容迁移的 schema v2 持久化 `music-library.json`，管理本地/网络曲目、音乐源、歌单、当前队列、去重、路径与 URL 校验、有界读取/下载以及公开元数据映射。go-music-dl adapter 读取 `/healthz`、`/collections?include_imported=1`、`/collections/:id/songs`，并通过 `/api/playlist/sources`、`/api/playlist/categories`、`/api/playlist/recommend`、`/api/playlist/search`、`/api/playlist/category`、`/api/playlist/songs` 浏览平台在线歌单，最后通过 `/download?stream=1` 取得音频。
