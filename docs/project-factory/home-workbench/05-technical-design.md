...
## 安全与生命周期
天气搜索参数编码，不接受任意 URL；超时与响应限制；错误脱敏；不自动定位。音乐文件或文件夹由系统对话框授权，支持扩展名、大小和普通文件校验；文件夹按稳定顺序递归扫描，忽略符号链接，限制为 10,000 个目录项和 20 层；网络音乐仅允许无凭据的公开 HTTPS URL，拒绝本机/私网解析和重定向，请求绑定验证后的公网地址并按流限制 128 MB。渲染层切换或删除当前来源、页面卸载时停止音频并撤销 Blob URL。AI沿用安全传输、全应用并发与取消；工作区/配置变更使当前对话请求过期，不自动读取本地资料。清空会话取消请求，迟到回复不回填。
