    ...
    setTimeout(async () => {
      try {
        ...
        const homeAudit=await contents.executeJavaScript(`(async()=>{
          ...
          return {saved,weather,weatherDetail,cleared,media,musicPanelStable,markdownSafe,composerGrows,pickerOpened,pickerFits,clipImageExcluded,contextSelected,contextSent,contextFrozen,chatted,chatNoteSaved:Boolean(chatNoteSaved),chatNoteUndone,cancelled,recoverable:Boolean(recoverable),retryRecovered,chatFits,recoverDebug,clearChat,classicRemoved,fits};
        })()`);
        assert.equal(homeAudit.recoverable,true,JSON.stringify(homeAudit.recoverDebug));delete homeAudit.recoverDebug;
        assert.deepEqual(homeAudit,{saved:true,weather:true,weatherDetail:true,cleared:true,media:true,musicPanelStable:true,markdownSafe:true,composerGrows:true,pickerOpened:true,pickerFits:true,clipImageExcluded:true,contextSelected:true,contextSent:true,contextFrozen:true,chatted:true,chatNoteSaved:true,chatNoteUndone:true,cancelled:true,recoverable:true,retryRecovered:true,chatFits:true,clearChat:true,classicRemoved:true,fits:true});
