...
const errors = [];
setTimeout(() => { console.error('Production startup timed out', errors); app.exit(1); }, 60000);
    ...
    setTimeout(async () => {
      try {
        ...
        const launcherState = await contents.executeJavaScript(`(async () => {
          ...
          while (document.activeElement !== input && performance.now() < focusDeadline) await new Promise(resolve => setTimeout(resolve, 20));
        ...
        const extensionRun = await contents.executeJavaScript(`(async () => {
          ...
          do {
            ...
            row = response.items?.find(item => item.title === 'PRODUCTION');
        ...
        let extensionClipboard='';const extensionClipboardDeadline=Date.now()+500;do{extensionClipboard=await require('electron').clipboard.readText();if(extensionClipboard!=='PRODUCTION')await new Promise(resolve=>setTimeout(resolve,20));}while(extensionClipboard!=='PRODUCTION'&&Date.now()<extensionClipboardDeadline);assert.equal(extensionClipboard,'PRODUCTION');
        ...
        try {
          ...
          assert.equal(accepted.ok,true);let acceptedClipboard='';const clipboardDeadline=Date.now()+500;do{acceptedClipboard=await require('electron').clipboard.readText();if(acceptedClipboard!=='CONFIRMED')await new Promise(resolve=>setTimeout(resolve,20));}while(acceptedClipboard!=='CONFIRMED'&&Date.now()<clipboardDeadline);assert.equal(acceptedClipboard,'CONFIRMED');
        ...
        const financeIpc=require('electron').ipcMain;
        const financeChannels=['finance:get-settings','finance:set-provider','finance:set-refresh-interval','finance:set-activity','finance:test-provider','finance:refresh','finance:overview','finance:ranking','finance:quotes','finance:history','finance:fundamentals','finance:cancel','finance:search'];
        for(const channel of financeChannels)financeIpc.removeHandler(channel);
        const financeProviderSettings={ok:true,schemaVersion:2,refreshSeconds:60,secureStorage:true,providers:[{id:'coingecko',label:'CoinGecko',market:'crypto',enabled:true,configured:true,credentialSource:'none',feed:'聚合 REST',state:'ready',hasCredential:false,verification:null},{id:'binance',label:'Binance',market:'crypto',enabled:true,configured:true,credentialSource:'none',feed:'公共 REST',state:'ready',hasCredential:false,verification:null},{id:'alpha-vantage',label:'Alpha Vantage',market:'us',enabled:false,configured:false,credentialSource:'none',feed:'Top Movers',state:'disabled',hasCredential:false,verification:null},{id:'alpaca',label:'Alpaca',market:'us',enabled:false,configured:false,credentialSource:'none',feed:'IEX · 单一交易所',state:'disabled',hasCredential:false,verification:null},{id:'twelve-data',label:'Twelve Data',market:'us',enabled:true,configured:true,credentialSource:'stored',feed:'Basic US feed · 非 SIP',state:'ready',hasCredential:true,verification:null},{id:'sec-edgar',label:'SEC EDGAR',market:'us',enabled:true,configured:true,credentialSource:'stored',feed:'官方申报 XBRL · 非行情',state:'ready',hasCredential:true,verification:null},{id:'cn-stock',label:'QuantDash',market:'cn',enabled:true,configured:true,credentialSource:'stored',feed:'实时快照 / 日 K',state:'ready',hasCredential:true,verification:{state:'verified',verifiedAt:'2026-09-15T09:00:00.000Z',error:'',capabilities:{realtime:'available',daily:'available',exactCodeSearch:'available',history:'available',metadata:'available',fullMarket:'permission_required',marketCap:'unsupported'}}},{id:'cn-tencent',label:'腾讯公开行情',market:'cn',enabled:true,configured:true,credentialSource:'none',feed:'公开行情 · GBK',state:'ready',hasCredential:false,verification:null},{id:'cn-eastmoney',label:'东方财富公开行情',market:'cn',enabled:true,configured:true,credentialSource:'none',feed:'公开行情 · JSON',state:'ready',hasCredential:false,verification:null},{id:'cn-sina',label:'新浪公开行情',market:'cn',enabled:true,configured:true,credentialSource:'none',feed:'公开行情 · GBK',state:'ready',hasCredential:false,verification:null}]};
        const ethAsset={id:'crypto:coingecko:ethereum',provider:'coingecko',providerAssetId:'ethereum',market:'crypto',type:'crypto',symbol:'ETH',name:'Ethereum',exchange:'CoinGecko',currency:'USD'};
        const ethQuote={asset:ethAsset,price:3210.42,changeAmount:32.1,changePercent:1.01,change1h:.2,change7d:4.3,marketCap:386000000000,marketCapRank:2,volume24h:15400000000,high24h:3260,low24h:3130,sparkline:[3000,3040,3025,3110,3160,3210.42],session:'continuous',feed:'coingecko-aggregate-rest',eventAt:'2026-09-15T08:59:00.000Z',retrievedAt:'2026-09-15T09:00:00.000Z',freshness:'delayed'};
        const btcAsset={id:'crypto:coingecko:bitcoin',provider:'coingecko',providerAssetId:'bitcoin',market:'crypto',type:'crypto',symbol:'BTC',name:'Bitcoin',exchange:'CoinGecko',currency:'USD'};
        const btcQuote={asset:btcAsset,price:67845.12,changeAmount:-1665.4,changePercent:-2.4,change1h:-.4,change7d:1.8,marketCap:1340000000000,marketCapRank:1,volume24h:34200000000,high24h:70120,low24h:66780,sparkline:[69200,70400,68100,68800,67000,67845.12],session:'continuous',feed:'coingecko-aggregate-rest',eventAt:'2026-09-15T08:58:00.000Z',retrievedAt:'2026-09-15T09:00:00.000Z',freshness:'delayed'};
        const solAsset={id:'crypto:coingecko:solana',provider:'coingecko',providerAssetId:'solana',market:'crypto',type:'crypto',symbol:'SOL',name:'Solana',exchange:'CoinGecko',currency:'USD'};
        const solQuote={asset:solAsset,price:146.33,changeAmount:8.02,changePercent:5.8,change1h:1.1,change7d:9.2,marketCap:68000000000,marketCapRank:6,volume24h:4200000000,high24h:149.2,low24h:137.6,sparkline:[132,136,134,141,144,146.33],session:'continuous',feed:'coingecko-aggregate-rest',eventAt:'2026-09-15T08:57:00.000Z',retrievedAt:'2026-09-15T09:00:00.000Z',freshness:'delayed'};
        const bnbAsset={id:'crypto:coingecko:binancecoin',provider:'coingecko',providerAssetId:'binancecoin',market:'crypto',type:'crypto',symbol:'BNB',name:'BNB',exchange:'CoinGecko',currency:'USD'};
        const bnbQuote={asset:bnbAsset,price:602.1,changeAmount:3.6,changePercent:.6,change1h:.1,change7d:2.2,marketCap:88000000000,marketCapRank:4,volume24h:1500000000,high24h:610,low24h:594,sparkline:[589,595,592,600,598,602.1],session:'continuous',feed:'coingecko-aggregate-rest',eventAt:'2026-09-15T08:56:00.000Z',retrievedAt:'2026-09-15T09:00:00.000Z',freshness:'delayed'};
        const usAsset={id:'us:twelve-data:AAPL',provider:'twelve-data',providerAssetId:'AAPL',market:'us',type:'stock',symbol:'AAPL',name:'Apple Inc.',exchange:'NASDAQ',currency:'USD'};
        const usQuote={asset:usAsset,price:205.5,changeAmount:5.5,changePercent:2.75,change1h:null,change7d:null,marketCap:null,marketCapRank:null,volume24h:90000000,high24h:208,low24h:198,sparkline:[],session:'regular',feed:'twelve-data-us-basic-non-sip',eventAt:'2026-09-15T08:59:30.000Z',retrievedAt:'2026-09-15T09:00:00.000Z',freshness:'delayed'};
        const cnAsset={id:'cn:quantdash:600519.SH',provider:'quantdash',providerAssetId:'600519.SH',market:'cn',type:'stock',symbol:'600519',name:'贵州茅台',exchange:'SSE',currency:'CNY'};
        const cnQuote={asset:cnAsset,price:1500,changeAmount:30,changePercent:2.04,change1h:null,change7d:null,marketCap:null,marketCapRank:null,volume24h:100000000,high24h:1510,low24h:1470,sparkline:[],session:'market',feed:'quantdash-realtime-snapshot',eventAt:'2026-09-15T08:59:57.000Z',retrievedAt:'2026-09-15T09:00:00.000Z',freshness:'real_time'};
        const financeExtraQuotes=Array.from({length:47},(_,index)=>({...bnbQuote,asset:{...bnbAsset,id:`crypto:coingecko:test-${index}`,providerAssetId:`test-${index}`,symbol:`T${index}`,name:`Test Asset ${index}`},price:700+index,changePercent:.7+index/100,marketCap:50000000000-index*1000000,volume24h:900000000-index*1000}));
        const financeRankingQuotes=[ethQuote,btcQuote,solQuote,bnbQuote,...financeExtraQuotes];
        const sortFinanceRanking=(sort)=>financeRankingQuotes.slice().sort((left,right)=>sort==='losers'?left.changePercent-right.changePercent:sort==='market_cap'?right.marketCap-left.marketCap:sort==='volume'?right.volume24h-left.volume24h:right.changePercent-left.changePercent);
        const paginateFinanceRanking=(rows,payload)=>{const pageSize=Math.max(1,Math.min(50,Number(payload.pageSize)||50));const totalRows=rows.length,totalPages=Math.max(1,Math.ceil(totalRows/pageSize)),page=Math.max(1,Math.min(totalPages,Number(payload.page)||1));const changeRows=rows.filter(row=>Number.isFinite(Number(row.changePercent))),positive=changeRows.filter(row=>row.changePercent>.05).length,negative=changeRows.filter(row=>row.changePercent<-.05).length;return {rows:rows.slice((page-1)*pageSize,page*pageSize),page,pageSize,totalRows,totalPages,hasMore:page<totalPages,counts:{total:totalRows,positive,negative,flat:changeRows.length-positive-negative}};};
        let lastFinanceActivity=null,lastFinanceAiPayload=null,financeAiCallCount=0,financeOverviewRetrievedAt='2026-09-15T09:00:00.000Z';
        financeIpc.handle('finance:get-settings',()=>financeProviderSettings);
        financeIpc.handle('finance:set-provider',()=>financeProviderSettings);
        financeIpc.handle('finance:set-refresh-interval',()=>financeProviderSettings);
        financeIpc.handle('finance:set-activity',(event,payload)=>{lastFinanceActivity=payload;return {ok:true};});
        financeIpc.handle('finance:test-provider',()=>({ok:true,settings:financeProviderSettings}));
        financeIpc.handle('finance:refresh',()=>({ok:true}));
        financeIpc.handle('finance:overview',()=>({ok:true,providers:financeProviderSettings.providers,retrievedAt:financeOverviewRetrievedAt,warnings:[],markets:[{market:'cn',label:'A 股',state:'available_without_summary',provider:'QuantDash',feed:'实时快照 / 日 K',session:'交易时段'},{market:'us',label:'美股',state:'available_without_summary',provider:'Twelve Data',feed:'Basic US feed · 非 SIP'},{market:'crypto',label:'加密货币',state:'available',provider:'CoinGecko',feed:'聚合 REST',session:'24/7',value:2500000000000,changePercent:1.2}],crypto:{marketCap:2500000000000,volume24h:100000000000,changePercent24h:1.2,activeAssets:14000,btcDominance:53.4,globalSeries:[{at:'2026-09-08T09:00:00.000Z',value:2400000000000},{at:'2026-09-09T09:00:00.000Z',value:2420000000000},{at:'2026-09-10T09:00:00.000Z',value:2390000000000},{at:'2026-09-11T09:00:00.000Z',value:2470000000000},{at:'2026-09-12T09:00:00.000Z',value:2490000000000},{at:'2026-09-15T09:00:00.000Z',value:2500000000000}],globalHistoryError:'',stale:false}}));
        financeIpc.handle('finance:ranking',(event,payload)=>payload.market==='us'?{ok:true,market:'us',source:'alpha-vantage',sort:payload.sort,rows:[usQuote],providers:financeProviderSettings.providers,page:1,pageSize:50,totalRows:1,totalPages:1,hasMore:false,counts:{total:1,positive:1,negative:0,flat:0},stale:false,retrievedAt:'2026-09-15T09:00:00.000Z'}:payload.market==='cn'?{ok:true,market:'cn',source:'quantdash',sort:payload.sort,rows:[cnQuote],coverage:['quantdash_cn_stock_universe'],providers:financeProviderSettings.providers,page:1,pageSize:50,totalRows:1,totalPages:1,hasMore:false,counts:{total:1,positive:1,negative:0,flat:0},stale:false,retrievedAt:'2026-09-15T09:00:00.000Z'}:{ok:true,market:payload.market,source:payload.source||'coingecko',sort:payload.sort,...paginateFinanceRanking(sortFinanceRanking(payload.sort),payload),providers:financeProviderSettings.providers,stale:false,retrievedAt:'2026-09-15T09:00:00.000Z'});
        financeIpc.handle('finance:quotes',()=>({ok:true,rows:[ethQuote],unavailable:[],errors:[],providers:financeProviderSettings.providers,retrievedAt:'2026-09-15T09:00:00.000Z'}));
        financeIpc.handle('finance:history',(event,payload)=>({ok:true,assetId:payload.assetId,days:payload.days,provider:payload.assetId.startsWith('us:')?'twelve-data':'coingecko',feed:payload.assetId.startsWith('us:')?'twelve-data-history':'coingecko-aggregate-rest',stale:false,series:[{at:'2026-09-08T09:00:00.000Z',value:payload.assetId.startsWith('us:')?198:3000},{at:'2026-09-09T09:00:00.000Z',value:payload.assetId.startsWith('us:')?200:3040},{at:'2026-09-10T09:00:00.000Z',value:payload.assetId.startsWith('us:')?201:3025},{at:'2026-09-11T09:00:00.000Z',value:payload.assetId.startsWith('us:')?203:3110},{at:'2026-09-12T09:00:00.000Z',value:payload.assetId.startsWith('us:')?204:3160},{at:'2026-09-15T09:00:00.000Z',value:payload.assetId.startsWith('us:')?205.5:3210.42}],retrievedAt:'2026-09-15T09:00:00.000Z'}));
        financeIpc.handle('finance:fundamentals',(event,payload)=>payload.assetId.startsWith('us:')?{ok:true,assetId:payload.assetId,provider:'sec-edgar',feed:'sec-edgar-xbrl-companyconcept',symbol:'AAPL',cik:'0000320193',entityName:'Apple Inc.',metrics:[{key:'assets',label:'总资产',concept:'Assets',value:350000000000,unit:'USD',periodStart:'',periodEnd:'2026-06-27',filedAt:'2026-08-01',form:'10-Q',fiscalYear:2026,fiscalPeriod:'Q3'},{key:'revenue',label:'营业收入',concept:'RevenueFromContractWithCustomerExcludingAssessedTax',value:94000000000,unit:'USD',periodStart:'2026-03-29',periodEnd:'2026-06-27',filedAt:'2026-08-01',form:'10-Q',fiscalYear:2026,fiscalPeriod:'Q3'}],stale:false,warning:'',retrievedAt:'2026-09-15T09:00:00.000Z'}:{ok:false,assetId:payload.assetId,error:'fundamentals_not_supported'});
        financeIpc.handle('finance:cancel',()=>({ok:true,cancelled:false}));
        financeIpc.handle('finance:search',(event,payload)=>{const query=String(payload.query);return query.toLowerCase().includes('eth')?{ok:true,items:[ethAsset],warnings:[],providers:financeProviderSettings.providers}:query==='600519'?{ok:true,items:[cnAsset],warnings:[],providers:financeProviderSettings.providers}:{ok:true,items:[],warnings:[],providers:financeProviderSettings.providers};});
        const financeSettings=await contents.executeJavaScript(`(async()=>{
          window.NotchSettings.select('finance');await new Promise(resolve=>requestAnimationFrame(resolve));
          ...
          const quantDashCapabilities=document.querySelector('[data-finance-provider-capabilities="cn-stock"]')?.textContent||'';
          const fixtureFields=Object.values(watchlists.assets).some(asset=>'base' in asset||'change' in asset||'price' in asset);
          return {selected:document.querySelector('[data-settings-category="finance"]').getAttribute('aria-selected')==='true',visible:!card.hidden,searchResultVisible,quantDashSearch,preferences,assetAdded:watchlists.lists[0].assetIds.includes('crypto:coingecko:ethereum'),providerForms,capabilityBoundary:quantDashCapabilities.includes('实时快照可用')&&quantDashCapabilities.includes('A 股全市场榜单需额外权限')&&quantDashCapabilities.includes('市值榜不提供'),fixtureFields,fits:content.scrollWidth<=content.clientWidth+1};
        })()`);
        assert.deepEqual(financeSettings,{selected:true,visible:true,searchResultVisible:true,quantDashSearch:true,preferences:{defaultView:'ranking',defaultMarket:'crypto',defaultSource:'coingecko',defaultRanking:'losers',refreshSeconds:0},assetAdded:true,providerForms:true,capabilityBoundary:true,fixtureFields:false,fits:true});
        fs.writeFileSync(path.join(__dirname,'../dist.noindex/finance-settings-review.png'),(await contents.capturePage()).toPNG());
        await contents.executeJavaScript(`document.querySelector('[data-finance-provider="twelve-data"]').scrollIntoView({block:'start'})`);await new Promise(resolve=>setTimeout(resolve,120));
        fs.writeFileSync(path.join(__dirname,'../dist.noindex/finance-us-providers-settings-review.png'),(await contents.capturePage()).toPNG());
        await contents.executeJavaScript(`document.querySelector('[data-finance-provider="cn-stock"]').scrollIntoView({block:'start'})`);await new Promise(resolve=>setTimeout(resolve,120));
        fs.writeFileSync(path.join(__dirname,'../dist.noindex/finance-quantdash-settings-review.png'),(await contents.capturePage()).toPNG());
        await contents.executeJavaScript(`document.querySelector('.finance-settings-watchlists').scrollIntoView({block:'start'})`);await new Promise(resolve=>setTimeout(resolve,120));
        fs.writeFileSync(path.join(__dirname,'../dist.noindex/finance-watchlists-settings-review.png'),(await contents.capturePage()).toPNG());
        await contents.executeJavaScript("window.__startupTestStage='finance-layout'");
        const financeLayout=await contents.executeJavaScript(`(async()=>{
          await window.NotchPanel.navigate({tab:'finance'});const deadline=performance.now()+2000;while((document.querySelector('[data-tab="finance"]').getAttribute('aria-selected')!=='true'||document.querySelectorAll('.finance-ranking-row').length<50)&&performance.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));
          const page=document.querySelector('.finance-page'),head=document.querySelector('.finance-page-head'),content=document.querySelector('.finance-content');
          const pageRect=page.getBoundingClientRect(),headRect=head.getBoundingClientRect(),contentRect=content.getBoundingClientRect();
          const rankingText=document.getElementById('finance-ranking-list').textContent;
          const result={active:document.querySelector('[data-tab="finance"]').getAttribute('aria-selected')==='true',view:document.querySelector('[data-finance-view][aria-selected="true"]')?.dataset.financeView,market:document.querySelector('[data-finance-market].active')?.dataset.financeMarket,ranking:document.getElementById('finance-ranking-sort').value,rankingRows:document.querySelectorAll('.finance-ranking-row').length,rankingSpotlight:document.querySelectorAll('.finance-ranking-spotlight-card').length,rankingTotal:document.getElementById('finance-ranking-total').textContent,noManagement:!page.querySelector('#finance-settings-asset-search,#finance-settings-add,#finance-settings-create-group,.finance-remove'),noPermanentSidebar:!page.querySelector('.finance-sidebar,.finance-main'),workspaceStacked:headRect.bottom<=contentRect.top+1,contained:headRect.left>=pageRect.left&&contentRect.right<=pageRect.right,realQuote:rankingText.includes('Ethereum')&&rankingText.includes('3,210.42'),sourceVisible:rankingText.includes('coingecko-aggregate-rest')};
          document.querySelector('[data-finance-view="overview"]').click();return result;
        })()`);
        assert.deepEqual(financeLayout,{active:true,view:'ranking',market:'crypto',ranking:'losers',rankingRows:50,rankingSpotlight:3,rankingTotal:'51',noManagement:true,noPermanentSidebar:true,workspaceStacked:true,contained:true,realQuote:true,sourceVisible:true});
        const financePagination=await contents.executeJavaScript(`(async()=>{document.getElementById('finance-ranking-next').click();const deadline=performance.now()+1500;while(document.getElementById('finance-ranking-page-label').textContent!=='第 2 / 2 页 · 51-51 / 51'&&performance.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));return {label:document.getElementById('finance-ranking-page-label').textContent,rowCount:document.querySelectorAll('.finance-ranking-row').length,firstRank:document.querySelector('.finance-ranking-index')?.textContent,previousDisabled:document.getElementById('finance-ranking-previous').disabled,nextDisabled:document.getElementById('finance-ranking-next').disabled};})()`);
        assert.deepEqual(financePagination,{label:'第 2 / 2 页 · 51-51 / 51',rowCount:1,firstRank:'51',previousDisabled:false,nextDisabled:true});
        assert.equal(lastFinanceActivity?.rankingPage,2);
        await contents.executeJavaScript(`(async()=>{document.getElementById('finance-ranking-previous').click();const deadline=performance.now()+1500;while(!document.getElementById('finance-ranking-page-label').textContent.startsWith('第 1 / 2 页')&&performance.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));})()`);
        await new Promise(resolve=>setTimeout(resolve,120));
        fs.writeFileSync(path.join(__dirname,'../dist.noindex/finance-overview-review.png'),(await contents.capturePage()).toPNG());
        const aShareRanking=await contents.executeJavaScript(`(async()=>{document.querySelector('[data-finance-view="ranking"]').click();document.querySelector('[data-finance-market="cn"]').click();const deadline=performance.now()+1500;while((document.querySelector('[data-finance-market].active')?.dataset.financeMarket!=='cn'||document.querySelectorAll('.finance-ranking-row').length!==1)&&performance.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));const text=document.getElementById('finance-ranking-list').textContent;return {market:document.querySelector('[data-finance-market].active')?.dataset.financeMarket,title:document.getElementById('finance-ranking-title').textContent,rowCount:document.querySelectorAll('.finance-ranking-row').length,hasMoutai:text.includes('贵州茅台')&&text.includes('1,500.00'),sourceHidden:document.getElementById('finance-ranking-source-label').hidden};})()`);
        assert.deepEqual(aShareRanking,{market:'cn',title:'A 股 · 24h 跌幅',rowCount:1,hasMoutai:true,sourceHidden:true});
        fs.writeFileSync(path.join(__dirname,'../dist.noindex/finance-cn-ranking-review.png'),(await contents.capturePage()).toPNG());
        await contents.executeJavaScript(`(async()=>{document.querySelector('[data-finance-market="crypto"]').click();const deadline=performance.now()+1500;while((document.querySelector('[data-finance-market].active')?.dataset.financeMarket!=='crypto'||document.querySelectorAll('.finance-ranking-row').length<50)&&performance.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));})()`);
        fs.writeFileSync(path.join(__dirname,'../dist.noindex/finance-ranking-review.png'),(await contents.capturePage()).toPNG());
        const rankingDetail=await contents.executeJavaScript(`(async()=>{const row=document.querySelector('.finance-ranking-row[data-finance-ranking-asset]');row?.click();const deadline=performance.now()+1500;while((document.querySelector('[data-finance-view][aria-selected="true"]')?.dataset.financeView!=='watchlist'||!document.querySelector('#finance-detail h2'))&&performance.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));return {view:document.querySelector('[data-finance-view][aria-selected="true"]')?.dataset.financeView,detailTitle:document.querySelector('#finance-detail h2')?.textContent||'',detailText:document.getElementById('finance-detail').textContent};})()`);
        assert.equal(rankingDetail.view,'watchlist');assert.equal(rankingDetail.detailTitle,'Bitcoin');assert.match(rankingDetail.detailText,/BTC/);assert.match(rankingDetail.detailText,/67,845\.12/);
        fs.writeFileSync(path.join(__dirname,'../dist.noindex/finance-watchlist-review.png'),(await contents.capturePage()).toPNG());
        const usFundamentals=await contents.executeJavaScript(`(async()=>{document.querySelector('[data-finance-view="ranking"]').click();document.querySelector('[data-finance-market="us"]').click();let deadline=performance.now()+1500;while((document.querySelector('[data-finance-market].active')?.dataset.financeMarket!=='us'||!document.querySelector('.finance-ranking-row[data-finance-ranking-asset]'))&&performance.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));document.querySelector('.finance-ranking-row[data-finance-ranking-asset]')?.click();deadline=performance.now()+1500;while(!document.querySelector('.finance-fundamentals dd')&&performance.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));const detail=document.getElementById('finance-detail'),text=detail.textContent;return{title:detail.querySelector('h2')?.textContent||'',metrics:detail.querySelectorAll('.finance-fundamentals dd').length,hasSource:text.includes('SEC 申报基本面')&&text.includes('10-Q')&&text.includes('披露 2026-08-01'),hasFeed:text.includes('twelve-data-us-basic-non-sip'),unsafe:!!detail.querySelector('script,img,iframe'),fits:detail.scrollWidth<=detail.clientWidth+1};})()`);
        assert.deepEqual(usFundamentals,{title:'Apple Inc.',metrics:2,hasSource:true,hasFeed:true,unsafe:false,fits:true});
        await contents.executeJavaScript(`document.querySelector('.finance-fundamentals')?.scrollIntoView({block:'center'})`);await new Promise(resolve=>setTimeout(resolve,120));
        fs.writeFileSync(path.join(__dirname,'../dist.noindex/finance-us-fundamentals-review.png'),(await contents.capturePage()).toPNG());
        contents.enableDeviceEmulation({screenPosition:'mobile',screenSize:{width:900,height:616},viewPosition:{x:0,y:0},deviceScaleFactor:1,viewSize:{width:900,height:616},scale:1});await new Promise(resolve=>setTimeout(resolve,180));
        const financeAt900=await contents.executeJavaScript(`(()=>{const page=document.querySelector('.finance-page'),workspace=document.querySelector('.finance-watchlist-workspace'),detail=document.querySelector('.finance-detail');return {width:innerWidth,detailHidden:getComputedStyle(detail).display==='none',workspaceColumns:getComputedStyle(workspace).gridTemplateColumns.split(' ').filter(Boolean).length,noHorizontalOverflow:page.scrollWidth<=page.clientWidth+1};})()`);
        assert.deepEqual(financeAt900,{width:900,detailHidden:true,workspaceColumns:1,noHorizontalOverflow:true});
        fs.writeFileSync(path.join(__dirname,'../dist.noindex/finance-900-review.png'),(await contents.capturePage()).toPNG());
        contents.enableDeviceEmulation({screenPosition:'mobile',screenSize:{width:620,height:616},viewPosition:{x:0,y:0},deviceScaleFactor:1,viewSize:{width:620,height:616},scale:1});await new Promise(resolve=>setTimeout(resolve,180));
        const financeAt620=await contents.executeJavaScript(`(()=>{const page=document.querySelector('.finance-page'),head=document.querySelector('.finance-page-head'),tabs=document.querySelector('.finance-view-tabs');const headRect=head.getBoundingClientRect(),tabsRect=tabs.getBoundingClientRect();return {width:innerWidth,headerRows:getComputedStyle(head).gridTemplateRows.split(' ').filter(Boolean).length,tabsInside:tabsRect.left>=headRect.left&&tabsRect.right<=headRect.right&&tabsRect.bottom<=headRect.bottom+1,noHorizontalOverflow:page.scrollWidth<=page.clientWidth+1};})()`);
        assert.deepEqual(financeAt620,{width:620,headerRows:2,tabsInside:true,noHorizontalOverflow:true});
        fs.writeFileSync(path.join(__dirname,'../dist.noindex/finance-620-review.png'),(await contents.capturePage()).toPNG());
        contents.disableDeviceEmulation();await new Promise(resolve=>setTimeout(resolve,180));
        ...
        const homeAudit=await contents.executeJavaScript(`(async()=>{
          ...
          const audio=document.getElementById('home-music-audio'),progress=document.getElementById('home-media-progress'),progressMutations=[];const progressObserver=new MutationObserver((records)=>progressMutations.push(...records));progressObserver.observe(progress,{attributes:true,attributeFilter:['hidden'],attributeOldValue:true});document.querySelector('[data-home-media="next"]').click();let trackSwitchDeadline=performance.now()+3000;while((document.getElementById('home-media-title').textContent!=='下一首测试歌曲'||audio.paused)&&performance.now()<trackSwitchDeadline)await new Promise(resolve=>setTimeout(resolve,20));progressMutations.push(...progressObserver.takeRecords());progressObserver.disconnect();const musicTrackSwitchDebug={progressHidden:progress.hidden,hiddenTransitions:progressMutations.filter(record=>record.oldValue===null).length,title:document.getElementById('home-media-title').textContent,status:document.getElementById('home-media-status').textContent,paused:audio.paused,source:audio.currentSrc||audio.src||'',readyState:audio.readyState,networkState:audio.networkState,errorCode:audio.error?.code||0};const musicTrackSwitchStable=!musicTrackSwitchDebug.progressHidden&&musicTrackSwitchDebug.hiddenTransitions===0&&musicTrackSwitchDebug.title==='下一首测试歌曲'&&!musicTrackSwitchDebug.paused;
          document.getElementById('home-media-library').click();await pause();const sourceSelect=document.getElementById('music-source-select');let sourceReadyDeadline=performance.now()+5000;while(sourceSelect.disabled&&performance.now()<sourceReadyDeadline)await new Promise(resolve=>setTimeout(resolve,20));sourceSelect.value='catalog-test';sourceSelect.dispatchEvent(new Event('change',{bubbles:true}));let sourceSwitchDeadline=performance.now()+8000;while((audio.paused||document.getElementById('home-media-title').textContent!=='晨光'||document.getElementById('music-playlist-select').value!=='morning'||document.getElementById('home-media-status').textContent!=='播放中'||document.getElementById('music-category-select').options.length!==3)&&performance.now()<sourceSwitchDeadline)await new Promise(resolve=>setTimeout(resolve,20));
          const firstPlaylistDebug={paused:audio.paused,title:document.getElementById('home-media-title').textContent,playlistId:document.getElementById('music-playlist-select').value,status:document.getElementById('home-media-status').textContent,source:audio.currentSrc||audio.src||'',readyState:audio.readyState,networkState:audio.networkState,errorCode:audio.error?.code||0};
          const firstPlaylistPlaying=!firstPlaylistDebug.paused&&firstPlaylistDebug.title==='晨光'&&firstPlaylistDebug.playlistId==='morning'&&firstPlaylistDebug.status==='播放中';
          ...
          const playlistSelect=document.getElementById('music-playlist-select');audio.pause();await pause();let playlistReadyDeadline=performance.now()+3000;while(playlistSelect.disabled&&performance.now()<playlistReadyDeadline)await new Promise(resolve=>setTimeout(resolve,20));playlistSelect.value='evening';playlistSelect.dispatchEvent(new Event('change',{bubbles:true}));let playlistSwitchDeadline=performance.now()+3000;while((audio.paused||document.getElementById('home-media-title').textContent!=='夜航'||document.getElementById('home-media-source-label').textContent!=='聚合音乐'||document.querySelector('[data-music-source-panel="catalog"]').hidden)&&performance.now()<playlistSwitchDeadline)await new Promise(resolve=>setTimeout(resolve,20));
          ...
          const musicPlaylistSwitchDebug={firstPlaylistPlaying,firstPlaylistDebug,paused:audio.paused,title:document.getElementById('home-media-title').textContent,playlistId:playlistSelect.value,sourceLabel:document.getElementById('home-media-source-label').textContent,catalogHidden:document.querySelector('[data-music-source-panel="catalog"]').hidden,status:document.getElementById('home-media-status').textContent};
          ...
          input.value='流式测试';document.getElementById('home-chat-form').requestSubmit();await new Promise(resolve=>setTimeout(resolve,30));document.getElementById('home-chat-stop').click();let stopDeadline=performance.now()+1000,stoppedReply;do{stoppedReply=[...document.querySelectorAll('#home-chat-messages [data-role="assistant"]')].at(-1);if(stoppedReply?.dataset.state==='stopped'&&document.getElementById('home-chat-form').dataset.busy==='false')break;await new Promise(resolve=>setTimeout(resolve,20));}while(performance.now()<stopDeadline);
          ...
          return {saved,autoLink,manualNote,capturedLink,duplicateLinkPreserved,weather,weatherDetail,cleared,media,musicTrackSwitchStable,musicTrackSwitchDebug,coverLoaded,homeOnlineDiscovery,settingsCategoriesLoaded,onlinePlaylistDisplayed,musicPlaylistSwitch,musicPlaylistSwitchDebug,noImplicitHistory,markdownSafe,composerGrows,pickerOpened,pickerFits,clipImageExcluded,contextSelected,contextSent,contextFrozen,chatted,chatNoteSaved:Boolean(chatNoteSaved),chatNoteUndone,cancelled,recoverable:Boolean(recoverable),retryRecovered,readerEligibility,readerOpened,readerTodoNeedsSelection,readerFits,outlineMoved,readerFullCopied,readerCodeCopied,readerSelectionReady,readerSelectionCopied,readerNoteSaved,readerNoteUndone,readerTodoSource,readerTodoPreview,readerTodoAppliedUndone,readerReturned,readerFocusRestored,readerClosed,readerFullTodoReady,readerFullTodoSource,readerFullTodoFocusRestored,saveDisclosure,sessionSaved,sessionRenamed,sessionSearchWorks,sessionRestored,sessionPanelFits,chatFits,recoverDebug,clearChat,classicRemoved,fits};
        })()`);
        assert.equal(homeAudit.recoverable,true,JSON.stringify(homeAudit.recoverDebug));delete homeAudit.recoverDebug;
        assert.equal(homeAudit.musicTrackSwitchStable,true,JSON.stringify(homeAudit.musicTrackSwitchDebug));delete homeAudit.musicTrackSwitchDebug;
        assert.equal(homeAudit.musicPlaylistSwitch,true,JSON.stringify(homeAudit.musicPlaylistSwitchDebug));delete homeAudit.musicPlaylistSwitchDebug;
        ...
        const readerExpectedCopy=await contents.executeJavaScript(`(()=>{document.getElementById('home-chat-reader-copy').click();return JSON.parse(localStorage.getItem('notch-ai-chat-sessions-v1')).sessions[0].records.find(item=>item.answer.includes('阶段内容')).answer;})()`);
        const readerExpectedNormalized=readerExpectedCopy.replace(/\r\n/g,'\n');let readerCopiedText='';const readerCopyDeadline=Date.now()+1000;do{readerCopiedText=(await clipboard.readText()).replace(/\r\n/g,'\n');if(readerCopiedText!==readerExpectedNormalized)await new Promise(resolve=>setTimeout(resolve,20));}while(readerCopiedText!==readerExpectedNormalized&&Date.now()<readerCopyDeadline);
        assert.equal(readerCopiedText,readerExpectedNormalized);
        ...
        await contents.executeJavaScript("window.__startupTestStage='finance-ai-interpretation'");
        require('electron').ipcMain.removeHandler('ai:run');
        require('electron').ipcMain.handle('ai:run',async(_event,payload)=>{
          if(payload.action!=='financeInterpretation')return{ok:false,error:'not_configured'};
          lastFinanceAiPayload=payload;
          financeAiCallCount+=1;
          if(financeAiCallCount===2)await new Promise(resolve=>setTimeout(resolve,250));
          const lines=String(payload.context?.text||'').split('\n');
          const evidence=(prefix)=>lines.find(line=>line.startsWith(prefix))||'';
          return{ok:true,kind:'financeInterpretation',requestId:payload.requestId,stance:'mixed',summary:'当前 provider 样本同时包含上涨与下跌资产，市场表现分化。结论只适用于本次快照与其声明的覆盖范围。',signals:[{direction:'positive',text:'样本中存在相对偏强的资产。',evidence:{quote:evidence('[A1]')}},{direction:'negative',text:'市场宽度仍包含下跌资产。',evidence:{quote:evidence('[B1]')}},{direction:'neutral',text:'当前宽度不代表完整市场。',evidence:{quote:evidence('[B2]')}}],watchItems:[{text:'后续继续核对 provider 覆盖范围与缓存状态。',evidence:{quote:evidence('[L2]')}}]};
        });
        const financeAi=await contents.executeJavaScript(`(async()=>{await window.NotchPanel.navigate({tab:'finance'});document.querySelector('[data-finance-view="overview"]').click();let deadline=performance.now()+1500;while(!document.querySelector('.finance-overview-leader')&&performance.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));document.getElementById('finance-ai-analyze').click();deadline=performance.now()+1000;while(!document.getElementById('finance-ai-status').textContent.includes('已完成')&&performance.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));const result=document.getElementById('finance-ai-result'),scope=document.getElementById('finance-ai-scope').textContent;return{status:document.getElementById('finance-ai-status').textContent,scopeBound:scope.startsWith('快照 ')&&scope.endsWith(' · 不构成投资建议'),stance:result.querySelector('.finance-ai-conclusion strong')?.textContent,groups:result.querySelectorAll('.finance-ai-signal-group').length,evidence:result.querySelectorAll('.finance-ai-evidence').length,unsafe:!!result.querySelector('script,img,iframe'),noHorizontalOverflow:result.scrollWidth<=result.clientWidth+1};})()`);
        assert.deepEqual(financeAi,{status:'已完成 · 基于当前快照',scopeBound:true,stance:'分化',groups:3,evidence:4,unsafe:false,noHorizontalOverflow:true});
        assert.equal(lastFinanceAiPayload?.action,'financeInterpretation');assert.equal(lastFinanceAiPayload?.interactive,true);assert.equal(lastFinanceAiPayload?.context?.sourceType,'manual');assert.match(lastFinanceAiPayload?.context?.text||'',/\[B1\] 市场宽度/);assert.doesNotMatch(lastFinanceAiPayload?.context?.text||'',/API Key|Token|notch-finance-watchlists/);
        const retainedFinanceAi=await contents.executeJavaScript(`(async()=>{document.getElementById('finance-ai-analyze').click();await new Promise(resolve=>setTimeout(resolve,40));const result=document.getElementById('finance-ai-result'),scope=document.getElementById('finance-ai-scope').textContent;return{summary:result.querySelector('.finance-ai-summary')?.textContent||'',stance:result.querySelector('.finance-ai-conclusion strong')?.textContent||'',groups:result.querySelectorAll('.finance-ai-signal-group').length,state:result.dataset.state,busy:result.getAttribute('aria-busy'),scopeBound:scope.startsWith('正在生成新解读，显示上次解读 · 快照 ')&&scope.endsWith(' · 不构成投资建议'),status:document.getElementById('finance-ai-status').textContent};})()`);
        assert.deepEqual(retainedFinanceAi,{summary:'当前 provider 样本同时包含上涨与下跌资产，市场表现分化。结论只适用于本次快照与其声明的覆盖范围。',stance:'分化',groups:3,state:'updating',busy:'true',scopeBound:true,status:'正在基于当前快照整理'});
        const replacedFinanceAi=await contents.executeJavaScript(`(async()=>{const deadline=performance.now()+1000;while(!document.getElementById('finance-ai-status').textContent.includes('已完成')&&performance.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));const result=document.getElementById('finance-ai-result');return{state:result.dataset.state||'',busy:result.getAttribute('aria-busy'),stance:result.querySelector('.finance-ai-conclusion strong')?.textContent||''};})()`);
        assert.deepEqual(replacedFinanceAi,{state:'',busy:'false',stance:'分化'});
        financeOverviewRetrievedAt='2026-09-15T09:05:00.000Z';
        const staleFinanceAi=await contents.executeJavaScript(`(async()=>{document.getElementById('finance-refresh').click();const deadline=performance.now()+1000;while(!document.getElementById('finance-ai-status').textContent.includes('行情已更新')&&performance.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));const result=document.getElementById('finance-ai-result'),scope=document.getElementById('finance-ai-scope').textContent;return{summary:result.querySelector('.finance-ai-summary')?.textContent||'',state:result.dataset.state,scopeBound:scope.startsWith('行情已更新，显示上次解读 · 快照 ')&&scope.endsWith(' · 不构成投资建议'),status:document.getElementById('finance-ai-status').textContent};})()`);
        assert.deepEqual(staleFinanceAi,{summary:'当前 provider 样本同时包含上涨与下跌资产，市场表现分化。结论只适用于本次快照与其声明的覆盖范围。',state:'previous',scopeBound:true,status:'行情已更新，可重新生成解读'});
        await contents.executeJavaScript(`document.getElementById('finance-ai-analyze').click()`);
        await contents.executeJavaScript(`(async()=>{const deadline=performance.now()+1000;while(!document.getElementById('finance-ai-status').textContent.includes('已完成')&&performance.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));})()`);
        await contents.executeJavaScript(`document.querySelector('.finance-ai-panel').scrollIntoView({block:'start'})`);await new Promise(resolve=>setTimeout(resolve,120));
        fs.writeFileSync(path.join(__dirname,'../dist.noindex/finance-ai-review.png'),(await contents.capturePage()).toPNG());
        for(const [width,columns] of [[900,3],[700,1],[620,1]]){
          contents.enableDeviceEmulation({screenPosition:'mobile',screenSize:{width,height:616},viewPosition:{x:0,y:0},deviceScaleFactor:1,viewSize:{width,height:616},scale:1});await new Promise(resolve=>setTimeout(resolve,120));
          await contents.executeJavaScript(`document.querySelector('.finance-ai-panel').scrollIntoView({block:'start'})`);await new Promise(resolve=>setTimeout(resolve,80));
          const responsive=await contents.executeJavaScript(`(()=>{const page=document.querySelector('.finance-page'),panel=document.querySelector('.finance-ai-panel'),grid=document.querySelector('.finance-ai-signal-grid');return{width:innerWidth,columns:getComputedStyle(grid).gridTemplateColumns.split(' ').filter(Boolean).length,noHorizontalOverflow:page.scrollWidth<=page.clientWidth+1&&panel.scrollWidth<=panel.clientWidth+1};})()`);
          assert.deepEqual(responsive,{width,columns,noHorizontalOverflow:true});
          fs.writeFileSync(path.join(__dirname,`../dist.noindex/finance-ai-${width}-review.png`),(await contents.capturePage()).toPNG());
        }
        contents.disableDeviceEmulation();
        console.log('Production workspace, note attachment, home workbench, saved AI sessions, finance interpretation and launcher checks passed');
        app.quit();
      } catch (error) {
