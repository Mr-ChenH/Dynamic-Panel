    ...
    setTimeout(async () => {
      try {
        ...
        const homeAudit=await contents.executeJavaScript(`(async()=>{
          ...
          document.getElementById('home-chat-open').click();const input=document.getElementById('home-chat-input');
          ...
          const firstReply=document.querySelector('#home-chat-messages [data-role="assistant"]');
          ...
          const retryRecovered=retriedReply?.dataset.state==='complete'&&retriedReply.querySelector('.home-chat-message-body').textContent==='整理后的内容'&&document.querySelectorAll('#home-chat-messages [data-role="user"]').length===3;
          ...
          return {saved,weather,weatherDetail,cleared,media,musicPanelStable,markdownSafe,composerGrows,chatted,chatNoteSaved:Boolean(chatNoteSaved),chatNoteUndone,cancelled,recoverable:Boolean(recoverable),retryRecovered,chatFits,recoverDebug,clearChat,classicRemoved,fits};
        })()`);
        assert.equal(homeAudit.recoverable,true,JSON.stringify(homeAudit.recoverDebug));delete homeAudit.recoverDebug;
        assert.deepEqual(homeAudit,{saved:true,weather:true,weatherDetail:true,cleared:true,media:true,musicPanelStable:true,markdownSafe:true,composerGrows:true,chatted:true,chatNoteSaved:true,chatNoteUndone:true,cancelled:true,recoverable:true,retryRecovered:true,chatFits:true,clearChat:true,classicRemoved:true,fits:true});
        ...
        await contents.executeJavaScript(`document.getElementById('home-chat-input').value='你好';document.getElementById('home-chat-form').requestSubmit()`);
        await new Promise(resolve=>setTimeout(resolve,150));
